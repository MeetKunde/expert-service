...
TEST_CASE("Creating empty DependenciesBank", "[expertBackground]") {
  ...
  REQUIRE(emptyDependenciesBank.getPointPairsDependencies(DependenciesBank::PointsPairsDependencies::EQUAL_SEGMENTS).empty());
  REQUIRE(emptyDependenciesBank.getPointPairsDependencies(DependenciesBank::PointsPairsDependencies::MEDIAN).empty());

  REQUIRE(emptyDependenciesBank.getTriplePointsPairDependencies(DependenciesBank::TriplePointsPairDependencies::MID_SEGMENT).empty());
  ...
  REQUIRE(emptyDependenciesBank.getPolygonTypeDependencies().empty());
}
...
TEST_CASE("Creating DependenciesBank based on ShapesBank", "[expertBackground]") {
  ...
  for(const auto& equ: lengths) {
    ...
    for(const auto& expEqu: expectedLengths) {
      if(equ->getFirstObject() == expEqu[0] && equ->getSecondObject() == expEqu[1] && equ->basedOn.empty() &&
          equ->importance == IDependency::ImportanceLevel::LOW && equ->dependencyCategory == IDependency::Category::FORMULA &&
          equ->dependencyType == IDependency::Type::SEGMENT_LENGTH && equ->dependencyReason == IDependency::Reason::POINTS_ARE_THE_SAME) {
            found = true;
      }
    }
  ...
  for(const auto& equ: measures) {
    ...
    for(const auto& expEqu: expectedMeasures1) {
      if(equ->getFirstObject() == expEqu[0] && equ->getSecondObject() == expEqu[1] && equ->basedOn.empty() &&
          equ->importance == IDependency::ImportanceLevel::LOW && equ->dependencyCategory == IDependency::Category::FORMULA &&
          equ->dependencyType == IDependency::Type::ANGLE_MEASURE && equ->dependencyReason == IDependency::Reason::POINTS_ARE_THE_SAME) {
            found = true;
      }
    }
    ...
    for(const auto& expEqu: expectedMeasures2) {
      if(equ->getFirstObject() == expEqu[0] && equ->getSecondObject() == expEqu[1] && equ->basedOn.empty() &&
          equ->importance == IDependency::ImportanceLevel::LOW && equ->dependencyCategory == IDependency::Category::FORMULA &&
          equ->dependencyType == IDependency::Type::ANGLE_MEASURE && equ->dependencyReason == IDependency::Reason::ARMS_ARE_THE_SAME) {
            found = true;
      }
    }
