...
TEST_CASE("Adding EquationDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addEquation(expressionWithExternalVar1, expressionWithExternalVar2, reason1, dependentDependencies1,
                                       importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency
  REQUIRE(dependenciesBank.addLength(pointA.getId(), pointB.getId(), expressionWithExternalVar1, reason1, dependentDependencies1,
                                     importanceLevel1) == 1);
  ...
  REQUIRE(dependenciesBank.addConvexAngle(pointC.getId(), pointB.getId(), pointA.getId(), expressionWithExternalVar2,
                                          reason1, dependentDependencies1, importanceLevel1) == 1);
  ...
  REQUIRE(dependenciesBank.addEquation(expressionLenVar, expressionWithExternalVar1, reason1, dependentDependencies1,
                                       importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));

  // attempt to add the same dependency
  // but with other method
  REQUIRE(dependenciesBank.addEquation(expressionConvexAngleVar, expressionWithExternalVar2, reason1, dependentDependencies1,
                                       importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));

  // attempt to add the same dependency
  // but with other method
  REQUIRE(dependenciesBank.addEquation(expressionConcaveAngleVar, expressionWithExternalVar3, reason1, dependentDependencies1,
                                       importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));

  // attempt to add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addEquation(expressionWithExternalVar1, expressionWithExternalVar2, reason1, dependentDependencies2,
                                       importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));

  // attempt to add the same dependency
  // but with other expressions order
  REQUIRE(dependenciesBank.addEquation(expressionWithExternalVar2, expressionWithExternalVar1, reason1, dependentDependencies1,
                                       importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));

  // attempt to add the same dependency
  // but with other first expression
  REQUIRE(dependenciesBank.addEquation(expressionWithExternalVar3, expressionWithExternalVar2, reason1, dependentDependencies1,
                                       importanceLevel1) == 1);
  ...
  REQUIRE(equations.front()->getReasons().front() == reason1);
  REQUIRE(equations.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(equations.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(equations.front()->getImportanceLevels().front() == importanceLevel1);
  ...
  REQUIRE(lengths.back()->getReasons().front() == reason1);
  REQUIRE(lengths.back()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(lengths.back()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(lengths.back()->getImportanceLevels().front() == importanceLevel1);
  ...
  REQUIRE(measures.back()->getReasons().front() == reason1);
  REQUIRE(measures.back()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(measures.back()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(measures.back()->getImportanceLevels().front() == importanceLevel1);
...
TEST_CASE("Adding LinesDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addLinesDependency(lineA, lineB, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addLinesDependency(lineA, lineB, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  // but in other lines order
  REQUIRE(dependenciesBank.addLinesDependency(lineB, lineA, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency but with other
  // dependent dependencies and other importance
  REQUIRE(dependenciesBank.addLinesDependency(lineA, lineB, type1, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addLinesDependency(lineA, lineB, type1, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other second line
  REQUIRE(dependenciesBank.addLinesDependency(lineA, lineC, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other type
  REQUIRE(dependenciesBank.addLinesDependency(lineA, lineB, type2, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));
  ...
  REQUIRE(perpendiculars.size() == 2);
  REQUIRE(parallels.size() == 1);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == perpendiculars.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency2Id)->getId() == perpendiculars.back()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == parallels.front()->getId());
  ...
  REQUIRE(perpendiculars.front()->getReasons().front() == reason1);
  REQUIRE(perpendiculars.front()->getReasons().back() == reason2);
  REQUIRE(perpendiculars.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(perpendiculars.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(perpendiculars.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(perpendiculars.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(perpendiculars.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(perpendiculars.front()->getImportanceLevels().back() == importanceLevel2);
  ...
  REQUIRE(parallels.front()->getReasons().front() == reason1);
  REQUIRE(parallels.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(parallels.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(parallels.front()->getImportanceLevels().front() == importanceLevel1);
...
TEST_CASE("Adding CirclesDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addCirclesDependency(circleA, circleB, type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addCirclesDependency(circleA, circleB, type, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  // but in other circles order
  REQUIRE(dependenciesBank.addCirclesDependency(circleB, circleA, type, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency but with other
  // dependent dependencies and other importance
  REQUIRE(dependenciesBank.addCirclesDependency(circleA, circleB, type, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addCirclesDependency(circleA, circleB, type, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other second line
  REQUIRE(dependenciesBank.addCirclesDependency(circleA, circleC, type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));
  ...
  REQUIRE(tangentCircles.size() == 2);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == tangentCircles.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency2Id)->getId() == tangentCircles.back()->getId());
  ...
  REQUIRE(tangentCircles.front()->getReasons().front() == reason1);
  REQUIRE(tangentCircles.front()->getReasons().back() == reason2);
  REQUIRE(tangentCircles.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(tangentCircles.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(tangentCircles.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(tangentCircles.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(tangentCircles.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(tangentCircles.front()->getImportanceLevels().back() == importanceLevel2);
...
TEST_CASE("Adding LineCircleDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addLineCircleDependency(lineA, circleA, type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addLineCircleDependency(lineA, circleA, type, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other line
  REQUIRE(dependenciesBank.addLineCircleDependency(lineB, circleA, type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other circle
  REQUIRE(dependenciesBank.addLineCircleDependency(lineA, circleB, type, reason1, dependentDependencies2, importanceLevel2) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // attempt to add the same dependency but with other
  // dependent dependencies and other importance
  REQUIRE(dependenciesBank.addLineCircleDependency(lineA, circleA, type, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addLineCircleDependency(lineA, circleA, type, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = swiped line and circle
  REQUIRE(dependenciesBank.addLineCircleDependency(swipedLineAToCircleA, swipedCircleAToLineA, type, reason1, dependentDependencies1,
                                                   importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));
  ...
  REQUIRE(tangentLines.size() == 4);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == tangentLines.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency4Id)->getId() == tangentLines.back()->getId());
  ...
  REQUIRE(tangentLines.front()->getReasons().front() == reason1);
  REQUIRE(tangentLines.front()->getReasons().back() == reason2);
  REQUIRE(tangentLines.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(tangentLines.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(tangentLines.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(tangentLines.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(tangentLines.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(tangentLines.front()->getImportanceLevels().back() == importanceLevel2);
...
TEST_CASE("Adding CirclePolygonDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addCirclePolygonDependency(circleA, polygonA, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addCirclePolygonDependency(circleA, polygonA, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other circle
  REQUIRE(dependenciesBank.addCirclePolygonDependency(circleB, polygonA, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency other polygon
  REQUIRE(dependenciesBank.addCirclePolygonDependency(circleA, polygonB, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // attempt to add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addCirclePolygonDependency(circleA, polygonA, type1, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addCirclePolygonDependency(circleA, polygonA, type1, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other type
  REQUIRE(dependenciesBank.addCirclePolygonDependency(circleA, polygonA, type2, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));
  ...
  REQUIRE(escribedCircles.size() == 3);
  ...
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == escribedCircles.back()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency4Id)->getId() == inscribedCircles.front()->getId());
  ...
  REQUIRE(escribedCircles.front()->getReasons().front() == reason1);
  REQUIRE(escribedCircles.front()->getReasons().back() == reason2);
  REQUIRE(escribedCircles.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(escribedCircles.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(escribedCircles.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(escribedCircles.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(escribedCircles.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(escribedCircles.front()->getImportanceLevels().back() == importanceLevel2);
  ...
  REQUIRE(inscribedCircles.front()->getReasons().front() == reason1);
  REQUIRE(inscribedCircles.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(inscribedCircles.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(inscribedCircles.front()->getImportanceLevels().front() == importanceLevel1);
...
TEST_CASE("Adding PointsPairsDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addPointsPairsDependency(pointA, pointB, pointC, pointD, type1, reason1, dependentDependencies1,
                                                    importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));
  ...
  REQUIRE(dependenciesBank.addPointsPairsDependency(pointD, pointC, pointB, pointA, type1, reason1, dependentDependencies1,
                                                    importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  // but in other points order in first PointsPairModel
  REQUIRE(dependenciesBank.addPointsPairsDependency(pointB, pointA, pointC, pointD, type1, reason1, dependentDependencies1,
                                                    importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  // but in other points order in second PointsPairModel
  REQUIRE(dependenciesBank.addPointsPairsDependency(pointA, pointB, pointD, pointC, type1, reason1, dependentDependencies1,
                                                    importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency but with other
  // dependent dependencies and other importance
  REQUIRE(dependenciesBank.addPointsPairsDependency(pointA, pointB, pointC, pointD, type1, reason1, dependentDependencies2,
                                                    importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addPointsPairsDependency(pointA, pointB, pointC, pointD, type1, reason2, dependentDependencies2,
                                                    importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other second line
  REQUIRE(dependenciesBank.addPointsPairsDependency(pointA, pointB, pointE, pointF, type1, reason1, dependentDependencies1,
                                                    importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));
  ...
  REQUIRE(equalSegments.size() == 2);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == equalSegments.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency2Id)->getId() == equalSegments.back()->getId());
  ...
  REQUIRE(equalSegments.front()->getReasons().front() == reason1);
  REQUIRE(equalSegments.front()->getReasons().back() == reason2);
  REQUIRE(equalSegments.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(equalSegments.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(equalSegments.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(equalSegments.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(equalSegments.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(equalSegments.front()->getImportanceLevels().back() == importanceLevel2);
...
TEST_CASE("Adding AnglesDependencies to DependenciesBank", "[expertBackground]") {
  ...
  REQUIRE(dependenciesBank.addAnglesDependency(pointB.getId(), pointA.getId(), pointC.getId(), convexAngleType,
                                       ...
                                       type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addAnglesDependency(pointB.getId(), pointA.getId(), pointC.getId(), convexAngleType,
                                       pointF.getId(), pointA.getId(), pointG.getId(), convexAngleType,
                                       type, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));
  ...
  REQUIRE(dependenciesBank.addAnglesDependency(pointB.getId(), pointA.getId(), pointC.getId(), convexAngleType,
                                       pointD.getId(), pointA.getId(), pointE.getId(), convexAngleType,
                                       type, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addAnglesDependency(pointB.getId(), pointA.getId(), pointC.getId(), convexAngleType,
                                       pointD.getId(), pointA.getId(), pointE.getId(), convexAngleType,
                                       type, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));
  ...
  REQUIRE(equalAngles.size() == 3);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == equalAngles.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == equalAngles.back()->getId());
  ...
  REQUIRE(equalAngles.front()->getReasons().front() == reason1);
  REQUIRE(equalAngles.front()->getReasons().back() == reason2);
  REQUIRE(equalAngles.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(equalAngles.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(equalAngles.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(equalAngles.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(equalAngles.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(equalAngles.front()->getImportanceLevels().back() == importanceLevel2);
...
TEST_CASE("Adding PolygonsDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // add new dependency
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleABCD, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleABCD, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // add the same dependency
  // but with other polygons order
  REQUIRE(dependenciesBank.addPolygonsDependency(rectangleABCD, triangleABC, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // add the same dependency
  // but with other first polygon vertices permutation
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleBCA, rectangleABCD, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // add the same dependency
  // but with other first polygon vertices permutation
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleCAB, rectangleABCD, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // add the same dependency
  // but with other second polygon vertices permutation
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleBADC, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // add the same dependency
  // but with other second polygon vertices permutation
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleDCBA, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other second polygon vertices permutation
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleACBD, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleABCD, fixedPointsOrder, type1, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleABCD, fixedPointsOrder, type1, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other second polygon
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, triangleCDE, fixedPointsOrder, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other type
  REQUIRE(dependenciesBank.addPolygonsDependency(triangleABC, rectangleABCD, fixedPointsOrder, type2, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));
  ...
  REQUIRE(similarTriangles.size() == 3);
  ...
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == similarTriangles.back()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency4Id)->getId() == congruentTriangles.front()->getId());
  ...
  REQUIRE(similarTriangles.front()->getReasons().front() == reason1);
  REQUIRE(similarTriangles.front()->getReasons().back() == reason2);
  REQUIRE(similarTriangles.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(similarTriangles.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(similarTriangles.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(similarTriangles.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(similarTriangles.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(similarTriangles.front()->getImportanceLevels().back() == importanceLevel2);
  ...
  REQUIRE(congruentTriangles.front()->getReasons().front() == reason1);
  REQUIRE(congruentTriangles.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(congruentTriangles.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(congruentTriangles.front()->getImportanceLevels().front() == importanceLevel1);
...
TEST_CASE("Adding PolygonTypeDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addPolygonTypeDependency(triangleABC, polygonType1, type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addPolygonTypeDependency(triangleABC, polygonType1, type, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other polygon
  REQUIRE(dependenciesBank.addPolygonTypeDependency(rectangleABCD, polygonType1, type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency other polygon type
  REQUIRE(dependenciesBank.addPolygonTypeDependency(triangleABC, polygonType2, type, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // attempt to add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addPolygonTypeDependency(triangleABC, polygonType1, type, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addPolygonTypeDependency(triangleABC, polygonType1, type, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));
  ...
  REQUIRE(polygonTypes.size() == 3);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == polygonTypes.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == polygonTypes.back()->getId());
  ...
  REQUIRE(polygonTypes.front()->getReasons().front() == reason1);
  REQUIRE(polygonTypes.front()->getReasons().back() == reason2);
  REQUIRE(polygonTypes.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(polygonTypes.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(polygonTypes.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(polygonTypes.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(polygonTypes.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(polygonTypes.front()->getImportanceLevels().back() == importanceLevel2);
...
TEST_CASE("Adding LinePointPairDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addLinePointsPairDependency(lineA, pointA, pointB, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addLinePointsPairDependency(lineA, pointA, pointB, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other line
  REQUIRE(dependenciesBank.addLinePointsPairDependency(lineB, pointA, pointB, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other points pair
  REQUIRE(dependenciesBank.addLinePointsPairDependency(lineA, pointC, pointB, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // attempt to add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addLinePointsPairDependency(lineA, pointA, pointB, type1, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addLinePointsPairDependency(lineA, pointA, pointB, type1, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));
  ...
  REQUIRE(midPerpendiculars.size() == 3);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == midPerpendiculars.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == midPerpendiculars.back()->getId());
  ...
  REQUIRE(midPerpendiculars.front()->getReasons().front() == reason1);
  REQUIRE(midPerpendiculars.front()->getReasons().back() == reason2);
  REQUIRE(midPerpendiculars.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(midPerpendiculars.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(midPerpendiculars.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(midPerpendiculars.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(midPerpendiculars.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(midPerpendiculars.front()->getImportanceLevels().back() == importanceLevel2);
  REQUIRE(midPerpendiculars.front()->getObjectAsJson() == dependency1Json);
}
...
TEST_CASE("Adding LineAngleDependencies to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addLineAngleDependency(lineA, pointA, pointB, pointC, angleType1, type, reason1, dependentDependencies1,
                                                  importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));
  ...
  // new dependency = other line
  REQUIRE(dependenciesBank.addLineAngleDependency(lineB, pointA, pointB, pointC, angleType1, type, reason1, dependentDependencies1,
                                                  importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other AngleModel's point
  REQUIRE(dependenciesBank.addLineAngleDependency(lineA, pointC, pointB, pointD, angleType1, type, reason1, dependentDependencies1,
                                                  importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other AngleModel's point
  REQUIRE(dependenciesBank.addLineAngleDependency(lineA, pointC, pointD, pointC, angleType1, type, reason1, dependentDependencies1,
                                                  importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));

  // new dependency = other AngleModel's type
  REQUIRE(dependenciesBank.addLineAngleDependency(lineA, pointC, pointB, pointC, angleType2, type, reason1, dependentDependencies1,
                                                  importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 5));

  // attempt to add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addLineAngleDependency(lineA, pointA, pointB, pointC, angleType1, type, reason1, dependentDependencies2,
                                                  importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 5));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addLineAngleDependency(lineA, pointA, pointB, pointC, angleType1, type, reason2, dependentDependencies2,
                                                  importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 5));
  ...
  REQUIRE(bisectors.size() == 5);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == bisectors.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency5Id)->getId() == bisectors.back()->getId());
  ...
  REQUIRE(bisectors.front()->getReasons().front() == reason1);
  REQUIRE(bisectors.front()->getReasons().back() == reason2);
  REQUIRE(bisectors.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(bisectors.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(bisectors.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(bisectors.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(bisectors.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(bisectors.front()->getImportanceLevels().back() == importanceLevel2);
...
TEST_CASE("Adding TriplePointsPairDependencies to DependenciesBank", "[expertBackground]") {
  ...
  REQUIRE(dependenciesBank.addPointsPairPairPointsPairDependency(
      ...
      pointC, pointD,
      pointE, pointF,
      type, reason1, dependentDependencies1, importanceLevel1
  ) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));
  ...
  REQUIRE(dependenciesBank.addPointsPairPairPointsPairDependency(
      ...
      pointC, pointD,
      pointE, pointF,
      type, reason1, dependentDependencies2, importanceLevel2
  ) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));
  ...
  REQUIRE(dependenciesBank.addPointsPairPairPointsPairDependency(
      ...
      pointC, pointD,
      pointE, pointF,
      type, reason2, dependentDependencies2, importanceLevel2
  ) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));
  ...
  REQUIRE(midSegments.front()->getDependentDependencies().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(midSegments.front()->getDependentDependencies().at(i) == dependentDependencies1.at(i));
  }
...
TEST_CASE("Adding PolygonPointsPairDependency to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addPolygonPointsPairDependency(triangleABC, fixedPointsOrder, pointG, pointH, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addPolygonPointsPairDependency(triangleABC, fixedPointsOrder, pointG, pointH, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other polygon
  REQUIRE(dependenciesBank.addPolygonPointsPairDependency(triangleDEF, fixedPointsOrder, pointG, pointH, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency = other points pair
  REQUIRE(dependenciesBank.addPolygonPointsPairDependency(triangleABC, fixedPointsOrder, pointG, pointI, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // attempt to add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addPolygonPointsPairDependency(triangleABC, fixedPointsOrder, pointG, pointH, type1, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addPolygonPointsPairDependency(triangleABC, fixedPointsOrder, pointG, pointH, type1, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other type
  REQUIRE(dependenciesBank.addPolygonPointsPairDependency(triangleABC, fixedPointsOrder, pointG, pointH, type2, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));
  ...
  REQUIRE(altitudes.size() == 3);
  REQUIRE(medians.size() == 1);

  REQUIRE(dependenciesBank.getDependencyById(dependency1Id)->getId() == altitudes.front()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == altitudes.back()->getId());
  ...
  REQUIRE(altitudes.front()->getReasons().front() == reason1);
  REQUIRE(altitudes.front()->getReasons().back() == reason2);
  REQUIRE(altitudes.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(altitudes.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(altitudes.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(altitudes.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(altitudes.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(altitudes.front()->getImportanceLevels().back() == importanceLevel2);
  ...
  REQUIRE(medians.front()->getReasons().front() == reason1);
  REQUIRE(medians.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(medians.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(medians.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(medians.front()->getObjectAsJson() == dependency5Json);
}
...
TEST_CASE("Adding PolygonExpressionDependency to DependenciesBank", "[expertBackground]") {
  ...
  // new dependency
  REQUIRE(dependenciesBank.addPolygonExpressionDependency(triangleABC, fixedPointsOrder, expression1, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // attempt to add the same dependency
  REQUIRE(dependenciesBank.addPolygonExpressionDependency(triangleABC, fixedPointsOrder, expression1, type1, reason1, dependentDependencies1, importanceLevel1) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 1));

  // new dependency = other polygon
  REQUIRE(dependenciesBank.addPolygonExpressionDependency(triangleDEF, fixedPointsOrder, expression1, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 2));

  // new dependency other expression
  REQUIRE(dependenciesBank.addPolygonExpressionDependency(triangleABC, fixedPointsOrder, expression2, type1, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // attempt to add the same dependency
  // but with other dependent dependencies and importance
  REQUIRE(dependenciesBank.addPolygonExpressionDependency(triangleABC, fixedPointsOrder, expression1, type1, reason1, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other reason, other dependent dependencies and other importance
  REQUIRE(dependenciesBank.addPolygonExpressionDependency(triangleABC, fixedPointsOrder, expression1, type1, reason2, dependentDependencies2, importanceLevel2) == 0);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 3));

  // new dependency = other type
  REQUIRE(dependenciesBank.addPolygonExpressionDependency(triangleABC, fixedPointsOrder, expression1, type2, reason1, dependentDependencies1, importanceLevel1) == 1);
  REQUIRE(dependenciesBank.getDependenciesNumber() == (baseDependenciesNumber + 4));
  ...
  REQUIRE(dependenciesBank.getDependencyById(dependency3Id)->getId() == perimeters.back()->getId());
  REQUIRE(dependenciesBank.getDependencyById(dependency4Id)->getId() == areas.back()->getId());
  ...
  REQUIRE(perimeters.front()->getReasons().front() == reason1);
  REQUIRE(perimeters.front()->getReasons().back() == reason2);
  REQUIRE(perimeters.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(perimeters.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(perimeters.front()->getDependentDependencies().back().size() == dependentDependencies2.size());
  for(size_t i = 0; i < dependentDependencies2.size(); i++) {
    REQUIRE(perimeters.front()->getDependentDependencies().back().at(i) == dependentDependencies2.at(i));
  }
  REQUIRE(perimeters.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(perimeters.front()->getImportanceLevels().back() == importanceLevel2);
  REQUIRE(perimeters.front()->getObjectAsJson() == dependency1Json);
  ...
  REQUIRE(areas.front()->getReasons().front() == reason1);
  REQUIRE(areas.front()->getDependentDependencies().front().size() == dependentDependencies1.size());
  for(size_t i = 0; i < dependentDependencies1.size(); i++) {
    REQUIRE(areas.front()->getDependentDependencies().front().at(i) == dependentDependencies1.at(i));
  }
  REQUIRE(areas.front()->getImportanceLevels().front() == importanceLevel1);
  REQUIRE(areas.front()->getObjectAsJson() == dependency5Json);
}
