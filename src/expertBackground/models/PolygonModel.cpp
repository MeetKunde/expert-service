...
PolygonModel::PolygonModel(Type type, bool fixedPointsOrder, std::vector<std::string> vertices) : 
  type{type}, fixedPointsOrder{fixedPointsOrder}, verticesIds{std::move(vertices)} {}

json PolygonModel::getJson() const {
  return {{"type", static_cast<int>(type)}, 
          {"fixedPointsOrder", fixedPointsOrder},
...
bool operator==(const PolygonModel& polygonModel1, const PolygonModel& polygonModel2) {
  ...
  if(polygonModel1.fixedPointsOrder && polygonModel2.fixedPointsOrder) {
    for (size_t i = 0; i < size1; i++) {
      if (polygonModel1.verticesIds[i] != polygonModel2.verticesIds[i]) {
        return false;
      }
    }
